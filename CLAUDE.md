...
4. **[src/core/position-sync.ts](src/core/position-sync.ts)** (`PositionSync`) - the actual decision engine, keyed by the trader's `baseId`. Computes target margin (trader's margin % of _their_ balance, clamped via `risk-policy` into your equity band), diffs against tracked state, places the delta order. Also owns same-coin conflict detection/auto-adopt (see below) and full unclamped closes. For an already-tracked `baseId`, before computing that delta it cross-checks the REAL live HL position against `entry.marginUsd` - the user is always free to manually open/edit/close on the exchange directly, and every delta below is otherwise computed purely from internal state with no live cross-check at all. No real position left → `manual_close_detected`, permanently stop managing that `baseId` (never silently reopen it). Direction no longer matches → `manual_direction_change_detected`, same permanent stop. Size differs by ≥`MIN_ORDER_USD` → `resynced_to_live_position`, `entry.marginUsd` recalculated from the real size before the delta is computed, so the next order moves from where the position actually is - and, critically, that resynced size is now the new, final baseline (see `lastAppliedFraction` below), not a staging step toward re-chasing an absolute trader-% target.

   **Target is incremental, not absolute** - `targetMarginUsd` is `entry.marginUsd + (clampedFraction - entry.lastAppliedFraction) * equity`, not `clampedFraction * equity` recomputed from scratch every cycle. `PositionState.lastAppliedFraction` (types.ts) records the risk-clamped trader fraction actually used the last time an order executed for this `baseId`; only the *change* in that fraction since then drives a new order. This exists specifically because the absolute-target version actively fought manual position changes: a user topping up (or trimming) a tracked position directly on the exchange would get resynced to the new real size, then the SAME cycle immediately "corrected" right back toward the trader's unchanged % - silently undoing the user's own manual action and realizing a real loss on the round-trip (confirmed live 2026-08-12 on ARB/XRP - traced via Hyperliquid's own `userFills`, not just this daemon's logs, to a manual top-up moments before each unexplained "reduced"). A brand-new open or a just-auto-adopted position has no prior fraction to diff against (`lastAppliedFraction` unset) - that first evaluation still targets the full absolute amount (same as before); every cycle after tracks only the trader-driven delta. A pending sub-`MIN_ORDER_USD` delta is deliberately NOT folded into `lastAppliedFraction` until an order actually executes, mirroring how `entry.marginUsd` itself is only updated on a real fill - otherwise a tiny trader signal change could get silently dropped instead of accumulating toward the next real order.
5. **[src/services/cloid-codec.ts](src/services/cloid-codec.ts)** - pure functions, no I/O: `decodeCloidToBaseShortId`, `decodeCloidLeverage`. Decodes Invo's own client-order-id (`cloid`) format on Hyperliquid orders it places - see "Identifying a manually-mimicked trade via cloid decoding" below. This daemon's own orders never set `cloid` at all (confirmed by grep - `hyperliquid-client.ts` has no reference to it), so a value that decodes cleanly is a free, deterministic signal that Invo's own client placed that order, not a guess.
6. **[src/core/cloid-attribution.ts](src/core/cloid-attribution.ts)** - `resolveConflictByCloid` (same-coin conflict resolution, see below) and `discoverCloidAttributedCoins` (finds any live HL position not yet tracked, decodes its most recent fill's `cloid`, resolves trader/investment via one batched `/dex/trade` call). Both are cache-gated via `CloidAttributionStore` (`cloid_attribution_cache` table in `sentinel.db`, keyed by coin) so a coin already confirmed genuinely manual (undecodable cloid) at its current position size costs nothing on repeat cycles - `userFills`/`dex/trade` are only called when something is actually unexplained. `CycleFillsCache` (`src/services/cycle-fills-cache.ts`) memoizes one `getUserFills()` call per reconcile cycle, shared between `PositionSync`'s conflict resolution and this discovery step, so at most one HL fills call happens per cycle regardless of how many coins need resolving.
7. **[src/services/risk-policy.ts](src/services/risk-policy.ts)** - pure functions, no I/O: `clampMarginFraction`, `clampLeverage`, `resolvePortfolioRisk`. The entire risk philosophy lives here: never reject, only resize. `resolvePortfolioRisk` applies a per-portfolio margin-band override (see `.copy-portfolio-risk.json` below) on top of the global `.env` band - an invalid override (min > max, negative) is rejected entirely, falling back to the full global band rather than clamping into something arbitrary.
8. **[src/clients/hyperliquid-client.ts](src/clients/hyperliquid-client.ts)** (`HyperliquidClient`) - thin wrapper over the `hyperliquid` SDK + public `/info` REST. Has two load-bearing signing quirks tied to the phantom-agent-key setup - **do not touch without testing against a live order**: `reduce_only` must always be `false` (breaks signature recovery otherwise; reduces/closes are just opposite-direction orders that HL nets automatically), and `grouping` must always be `'na'` (also breaks signature recovery; this is also why there's no exchange-side TP/SL).
9. **[src/services/db.ts](src/services/db.ts)** - opens the shared `data/sentinel.db` (better-sqlite3, WAL mode so the UI's read-only connection is never blocked by the daemon's writes), runs idempotent `CREATE TABLE IF NOT EXISTS` migrations, and - once, only the moment the file is first created - imports whatever the old flat-JSON stores (`.copy-state.json`, `.copy-ignored.json`, `.copy-cloid-cache.json`, `.copy-followed-portfolios.json`) still have on disk. Chosen over Postgres/similar specifically for the Raspberry Pi deployment target (zero server process, zero ops); chosen over Node's own built-in `node:sqlite` because that's still flagged experimental as of Node 24 ("might change at any time" per its own runtime warning) - not a bet worth making for unattended trading state, versus `better-sqlite3`'s long-stable synchronous API, which also matches this codebase's existing sync-fs-call style everywhere else. `.copy-portfolio-risk.json` deliberately stays plain JSON, not migrated - it's the one user-hand-edited store (see `portfolio-risk-store.ts`), and a hand-editable file is a better fit for that than a DB row.
10. **[src/services/state-store.ts](src/services/state-store.ts)** - persists `PositionStateMap` to `position_state` in `sentinel.db`, keyed by trader `baseId`. `save()` keeps the old JSON file's "whole-map overwrite" contract (delete-all-then-reinsert, wrapped in one transaction so it's atomic - an improvement over the plain `writeFileSync` it replaces). Saved after every order attempt, not batched.
11. **[src/services/closed-trades-store.ts](src/services/closed-trades-store.ts)** - `ClosedTradesStore`, append-only history in `closed_trades` (`sentinel.db`) - the one table that survives both a position actually closing (`position_state`'s row for it is deleted at that point, same as the old JSON behavior) and that trade's portfolio later being unfollowed. `portfolioTitle` is snapshotted at `record()` time specifically so portfolio-level analytics still resolve a name after an unfollow, when the live followed-portfolios list no longer has it to offer - this is what the UI's trade-history/analytics grouping (`ui/server/history/buildTradeHistory.ts`) now reads first for any closed trade, falling back to its older log/fill reconstruction only for closes that predate this store. Written once per real close, from three call sites: `PositionSync.close()` (the normal reconciler-driven close and its `manual_close_detected`/`manual_direction_change_detected` variants) and `close-position.ts` (the manual emergency-close CLI).
12. **[src/services/logger.ts](src/services/logger.ts)** - JSON-line logger to `logs/<name>-YYYY-MM-DD.log` + stdout, self-enforcing retention (`LOG_RETENTION_HOURS`) and total-size cap (`LOG_MAX_TOTAL_MB`, oldest evicted first).
13. **[src/services/healthcheck.ts](src/services/healthcheck.ts)** - optional `HEALTHCHECK_PING_URL` pings (`/start` + success/`fail`, e.g. healthchecks.io) around each reconcile cycle. Fire-and-forget by design, never awaited from the main loop - a slow/unreachable monitor must never delay trading. The one exception is `pingFailAwaited`, used only from the fatal crash handlers with a short bounded timeout, since a true fire-and-forget ping there would very likely get killed by `process.exit()` before it left the machine.
14. **[src/services/stale-entry-policy.ts](src/services/stale-entry-policy.ts)** - pure functions, no I/O: `evaluateStaleEntry`, `computeInvestmentPnlPercent`. The one deliberate exception to risk-policy's "never reject, only resize": a trade already too old to mirror faithfully by the time it's about to be opened fresh gets skipped instead - permanently, on age alone, regardless of its PnL at that moment.
15. **[src/services/ignored-trades-store.ts](src/services/ignored-trades-store.ts)** - persists `IgnoredTradesMap` to `ignored_trades` in `sentinel.db`, its own table, separate from `position_state` on purpose: these baseIds were never opened, so they must stay out of the close-detection loop in `Reconciler.run()`, which issues real Hyperliquid closes for anything tracked but no longer in a trader's open list.
16. **[src/services/portfolio-risk-store.ts](src/services/portfolio-risk-store.ts)** - `PortfolioRiskStore`, persists `PortfolioRiskEntry[]` to `.copy-portfolio-risk.json`, a **user-editable** config file (unlike the other stores, which are machine-only and live in `sentinel.db`). `sync(followed)`, called every cycle from `Reconciler.run()` right after fetching the followed-portfolios list, adds a blank (`null`/`null`) entry for a newly-followed portfolio and drops one for anyone unfollowed - the file is only rewritten when something structural actually changes, never on a user's own `minMarginPct`/`maxMarginPct` edits alone, so hand-edited values are never clobbered by a routine cycle.
17. **[src/services/followed-portfolios-store.ts](src/services/followed-portfolios-store.ts)** - `FollowedPortfoliosStore`, machine-only, persists the full followed-portfolios list (one JSON blob per row, keyed by portfolio id) to `followed_portfolios` in `sentinel.db`, overwritten every cycle right after `Reconciler.run()` fetches it. Exists purely so the dashboard UI's right rail (`ui/server/daemon/loadFollowedPortfolios.ts`) can read this instead of calling Invo's API itself - confirmed live 2026-08-11 that the UI had its own independent `InvoClient` hitting Invo directly, sharing the same **IP-scoped** rate-limit budget as the daemon (see `poll-schedule.ts` below - it's per-IP via Cloudflare, not per-account) with no cache and no timeout; a 429 storm on the daemon side hung the UI widget for however long the daemon's own retry-after backoff took, on every page navigation. Reading this table instead removes the UI as an Invo caller entirely for this widget.
18. **[src/services/poll-schedule.ts](src/services/poll-schedule.ts)** - `computeSafePollIntervalMs`, pure function, no I/O. Confirmed live 2026-08-11 via a direct probe against `api.invoapp.com`: a 429 there carries Cloudflare's `ratelimit-policy: 500;w=300` and `Too many requests from this IP` - a 500-request-per-300-second budget scoped to this server's IP, not the Invo account. Each cycle makes `1 + N` Invo calls (the followed-portfolios list, then one `get_investments` per followed portfolio); past a certain N, that structurally exceeds the sustained 1.67 req/s budget on its own, with or without retries - not a transient blip that clears by waiting, confirmed by ~100+ minutes of zero recovery at N=14 before this fix. `auto-copy.ts`'s main loop calls this every cycle to compute the actual gap before the next one, scaling it up once `pollIntervalMs` alone would be unsafe at the current follow count - a no-op at typical follow counts, logged as `poll_interval_throttled` when it actually kicks in.
...
3. Right direction, shared with another follower → decode the real position's own order `cloid` (`cloid-attribution.ts`'s `resolveConflictByCloid`) and string-match it against each candidate's own `baseShortId` - exact, not an inference; see "Identifying a manually-mimicked trade via cloid decoding" below.
4. Still inconclusive (cloid doesn't decode - not an Invo-placed order at all) → logged as `existing_position_conflict`, left untouched until `npm run adopt` or manual close.
...
### Identifying a manually-mimicked trade via cloid decoding

A position the user opens directly through Invo's own app "Mimic" button (not via this daemon) needs to be discovered and attributed to a trader before it can be adopted. Reverse engineered 2026-08-13 from real Hyperliquid fills (confirmed exact on 7 real samples, 2 traders, 2 leverages): every order Invo's own client places carries a structured 16-byte `cloid` (Hyperliquid's client-order-id field) -

```
byte[0]     = 0x02 (constant)
byte[1]     = action marker (0x00 = open, 0x41/0x42 = increase, 0x31/0x32/0x51/0x52 = decrease/close variants)
byte[2]     = 0x00 (constant)
byte[3]     = leverage, raw integer
byte[4]     = 0x01 (constant)
byte[5]     = 0x00 (constant)
byte[6]     = 0x00 (constant)
byte[7:15]  = the trader's `baseShortId`, base62-decoded (alphabet 0-9A-Za-z) as a big-endian uint64
byte[15]    = per-order suffix, not identity-bearing
```

This daemon's own orders never set `cloid` at all (`hyperliquid-client.ts` has no reference to it), so a cloid that decodes cleanly against this exact layout is a free, deterministic signal that Invo's own client placed that order - not this daemon, and not some unrelated manual trade on the same wallet. `cloid-attribution.ts`'s `discoverCloidAttributedCoins` uses this every cycle to find any live HL position not yet tracked: decode its most recent fill's cloid, and if it decodes, resolve the trader/investment via one batched `/dex/trade` call (`creatorAppUserId`/`portfolioId`/`investmentBaseId` in the response). A cloid that doesn't decode means the position has no connection to Invo's mimic feature at all - left completely untouched, exactly as a manual trade should be.

This replaced an earlier notification/TP-SL-based heuristic system (`manual-mimic-tracker.ts`, deleted): Invo's own `/dex/trade` `isMimicked`/`unmimickedCount` fields, previously used both for same-coin conflict resolution and as a TP/SL cross-reference confirmation signal, turned out to answer "was this specific update mimicked" rather than "did you ever mimic this investment at all" - confirmed live 2026-08-13 on a real, certain case (STX/limpan96) where `isMimicked` stayed `false` throughout. Cloid decoding requires no such inference.

`CloidAttributionStore` (`cloid_attribution_cache` table in `sentinel.db`, keyed by coin) makes this cheap on repeat cycles: a coin already confirmed genuinely manual (undecodable cloid) at its current live position size is trusted without re-fetching anything, until the position's size changes (covering a close-then-reopen under a new mimic). `userFills`/`dex/trade` are only called when something is actually unexplained, and `CycleFillsCache` memoizes that one `getUserFills()` call for the whole reconcile cycle - so in the common steady-state case (nothing new to explain) this feature costs zero extra API calls, and even when something is unexplained it costs at most one HL call and one batched Invo call, never one per coin.
...
**You are: Mika** (ID: `f9bb0a8c-34f5-4cf8-a993-4f73e02d6733`)
...
Engineering work in this workspace is local-only by default. Runtimes should check out and edit repos in place on the local machine, working directly on whatever branch is already checked out (e.g. develop, or gitflow feature/bugfix branches) -- never spin up a worktree, never create a new agent-owned branch, never work in the cloud. Never push to a remote, never open a pull request, and never trigger CI/CD. Do not create git commits automatically as part of a task. Only commit when the user explicitly invokes a dedicated git-commit step/skill; until that exists, leave changes uncommitted in the working tree for the user to review and commit themselves. When a commit is authorized, it MUST be performed by invoking the git-commit skill itself -- never by running `git commit`/`git add` directly by hand. This is strict and non-negotiable: skipping the skill because the change looks trivial, because of time pressure, or because of ambiguity is not acceptable. Never add AI/LLM attribution, co-author trailers, 'Generated with...' lines, or any signature indicating AI authorship in commits, code, or comments.

Before doing repo-specific work -- especially anything touching branching, merging, or commit conventions -- always check that repo/project's own instruction files first: CLAUDE.md, AGENTS.md, or any other repo/dir-specific instructions present at its root. Treat these as extremely important: with multiple agents working the same repo in parallel across different issues, a repo's own stated conventions (e.g. "create a feature branch per task and merge+delete it once done") are what keeps that parallel work from turning into chaotic, conflicting branch/commit state -- follow them. That said, a repo's own instruction file does not automatically override this workspace context or an agent's own Agent Identity instructions wholesale -- when a repo's file conflicts with either of those, or the right course isn't clear, stop and ask (the requesting member, or Mika) rather than silently picking a side.

Project exception -- invo-sentinel (local dir C:\DEV\invo-sentinel): that repository's own CLAUDE.md is the authoritative source for all local git/branching/commit policy for this project, overriding the general workspace defaults above (e.g. it does permit agents to create and merge local feature/* and bugfix/* branches off develop). The one exception on top of that file: any agent may invoke the git-commit skill for this project, but only when directly instructed to commit in that same turn by a workspace member or by Mika (the workspace leader) -- an agent must never decide to commit on its own initiative.
...
- https://github.com/plungarini/isure-be.git
- https://github.com/plungarini/isure.git

## Project Context

The active project for this task is **invo-sentinel**.

Project description — durable context the project owner set for work in this project:

Copy trading bot for Hyperliquid that mirrors Invo followed traders, with equity-based position sizing and leverage caps.

Working constraints: work only in the local checkout at C:\DEV\invo-sentinel, in place on whatever branch is currently checked out (usually develop, or gitflow branches like feature/* or bugfix/*). Do not create a new worktree or workspace. Do not push, open PRs, or trigger CI. Do not create git commits automatically; leave changes uncommitted for manual review unless explicitly told to commit.

Project resources (also written to `.multica/project/resources.json`):

- **local_directory**: `{"daemon_id":"01a00fa6-e6ef-7aa4-bb82-00586bf291e0","local_path":"C:\\DEV\\invo-sentinel","execution_mode":"in_place"}`
