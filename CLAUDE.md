...
1. **[src/clients/invo-client.ts](src/clients/invo-client.ts)** (`InvoClient`) — reverse-engineered Invo REST wrapper. Long-lived refresh token traded for ~10min access tokens, auto-refreshed on expiry/401. **Source of truth is `get_investments(isOpen: true)` per portfolio, never the social feed** (feed has propagation delay + a server-side pagination bug beyond page 8; not implemented here at all). `recordOpen`'s `baseShortId` must be the same client-generated `ourBaseShortId` later sent to `recordClose` — Invo never assigns/returns its own short ID for a position we create, so if `recordOpen` doesn't tell it what ID to file the position under, `recordClose` 404s (`NOT_FOUND`) trying to close something Invo never associated with that ID; the real Hyperliquid close still succeeds either way, only Invo's own bookkeeping breaks.
...
HL rejects any order under $10 notional outright, independent of this project's own risk band. A brand-new open whose computed notional (clamped margin × leverage) lands under that floor is bumped up to exactly `HL_MIN_NOTIONAL_USD` (position-sync.ts) instead of being attempted at the smaller, doomed-to-reject size — otherwise it would just fail identically every cycle until the stale-entry rule above eventually gives up on it for having gone unfilled too long. Only applies to the initial open, not incremental top-ups (those are left to cross the floor naturally via `targetMarginUsd` drift, so small ongoing adjustments can't compound into a margin far past the configured band). Tracked `marginUsd` is always computed from the actual (rounded, possibly floor-bumped) filled size, not the pre-rounding target, so state matches what really executed.
